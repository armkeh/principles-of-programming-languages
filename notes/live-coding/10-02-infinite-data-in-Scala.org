...
of the form ~Unit => T~; that is,
the type of /functions/ from ~Unit~ to ~T~.
(Recall that ~Unit~ is the type with just one value,
written ~()~ in Scala.)
...
** Dealing with the tail function
:PROPERTIES:
:CUSTOM_ID: Dealing-with-the-tail-function
:END:

Given a stream of the form ~Cons(a,f)~,
the ~f~ parameter does not directly give us the “tail” stream;
we must invoke it, writing ~f()~, to obtain the tail.

In the other direction,
in order to construct a stream of the form ~Cons(a,f)~,
we must “wrap” the tail stream in a function definition.
Rather than writing out a ~defun~ and giving this function a name
(which in Scala lingo would make it a method),
we can use an /anonymous/ definition,
such as below.
#+begin_src amm :results none
val ones: Stream[Int] = Cons(1, _ => ones)
#+end_src
The ~_ => ones~ indicates a function of one argument;
the ~_~ is used in place of a variable name,
since we are not using the variable
(because it is of type ~Unit~, there is really no use in
giving it a name; it must be ~()~.)

#+begin_center
So, to construct a stream, we must “wrap” the tail;
this is also called *delaying* the tail.

To deconstruct (stream) a stream, we must “invoke” the tail;
this is also called *forcing* the tail.

*The concepts of delaying and forcing data are generally applicable;*
*we may later even consider a type which is just (potentially)*
*delayed data.*
#+end_center
...
Since there can only be one instance of ~SNil~,
...
meaning that ~Stream[A]~ is a subtype of ~Stream[B]~ if
~A~ is a subtype of ~B~.
...
in order to give a way to /deconstruct/ them.
...
:PROPERTIES:
:CUSTOM_ID: Take-care:-*make-arguments-of-type-~Stream~-by-name-arguments*
:END:

Notice that in the definition of ~prepend~,
...
use ~prepend~ to define a stream /recursively/,
so the recursive call is inserted as an argument
to ~prepend~ (e.g., ~val ones: Stream[Int] = repeat(List(1), ones)~),
if that argument were /not/ by name, it would be evaluated,
and we would get a stack overflow due to unbounded recursion
(this occurred during the lecture on Monday, October 5th.)

#+begin_center
*In general, mark your arguments of type ~Stream~ by name,*
*in order to avoid accidental evaluation.*
#+end_center

** More methods on our streams
:PROPERTIES:
:CUSTOM_ID: More-methods-on-our-streams
:END:

Using ~prepend~, let us define two ways
to go from a stream to a list;
either just convert the list to a finite stream
(one that actually does end with ~SNil~ eventually),
...
We can also /append/ a list to a stream, or more appropriately,
...
since we never reach the ~SNil~ case while moving
