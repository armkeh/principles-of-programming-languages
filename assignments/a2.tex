...
\label{sec:orga2da977}
\subsection*{November 20th}
\label{sec:org7b90b2e}
\begin{itemize}
...
\section*{Boilerplate}
\label{sec:org1774ddc}
\subsection*{Documentation}
\label{sec:orgbaf4694}
...
\label{sec:orgbd22738}
\subsubsection*{Scala}
\label{sec:orgbbfa7d4}
...
def emptyEnv; lambda { |i| throw "Typing an unbound variable" } end
  
def prependEnv(a, &env)
  lambda { |i| if i == 0 then a else env.call(i) end }
end
...
class STAbs < STTerm
  ...
  def initialize(a,t)
    unless a.is_a?(STType) && t.is_a?(STTerm)
      throw "Constructing a lambda term with a non-lambda term or a non-type"
    end
    @type = a
    @t = t
  end
  ...
  def to_s
    "lambda " + @type.to_s + " . " + @t.to_s
  end

  def typeOf(&env)
    STFun.new(@type,@t.typeOf(&prependEnv(@type,&env)))
  end

  def eraseTypes; ULAbs.new(@t.eraseTypes) end
end
...
class STApp < STTerm
  ...
  def typeOf(&env)
    t1_type = @t1.typeOf(&env)
    a1 = t1_type.dom
    b = t1_type.codom
    a2 = @t2.typeOf(&env)

    if a1 == a2 then b else throw "Applying the wrong type of value to an abstraction" end
  end
...
class STSuc < STTerm
  ...
  def to_s; "suc " + @t.to_s end
  def typeOf(&env)
    throw "Applying successor to a non-natural" unless @t.typeOf(&env) == STNat.new
    STNat.new
  end
  ...
  def eraseTypes
    ULApp.new(
      ULAbs.new( # lambda n
        ULAbs.new( # lambda s
          ULAbs.new( # lambda z
            ULApp.new(ULVar.new(1), # s
                      ULApp.new(ULApp.new(ULVar.new(2),  # n
                                          ULVar.new(1)), # s
                                ULVar.new(0)))))), # z
      @t.eraseTypes)
  end
...
class STIsZero < STTerm
  ...
  def initialize(t); @t = t end
  def to_s; "iszero " + @t.to_s end
  def typeOf(&env)
    throw "Applying iszero to a non-natural" unless @t.typeOf(&env) == STNat.new
    STBool.new
  end

  def eraseTypes
    tru = STTrue.eraseTypes
    fls = STFalse.eraseTypes
    test = ULAbs.new(ULApp.new(ULApp.new(ULVar.new(0),ULAbs.new(fls))),tru)
    
    ULApp(test,@t.eraseTypes)
  end
end
